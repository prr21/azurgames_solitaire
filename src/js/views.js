...
import { Stack, Card, RankValues, SuitValues } from "./model"
import { StackViewOptions, CardViewOptions } from "../types"

export class StackView extends Phaser.GameObjects.Container {
  _model: Stack
  _cardOffsetY: integer
  _cardOffsetX: integer

  constructor(stack: Stack, options: StackViewOptions) {
    const {
      scene,
      atlas = "cards",
      back = "back",
  ...
  get cardOffsetY() {
    return this._cardOffsetY
  }

  get cardOffsetX() {
    return this._cardOffsetX
  }
}
export class CardView extends Phaser.GameObjects.Container {
  ...
  constructor(card: Card, options: CardViewOptions) {
    const {
      scene,
      stackViews,
      cardViews,
  ...
  _updateState() {
    ...
    const prevCards = this._stackView.model.cards
      .filter((i) => i.order < this._model.order)
      .sort((a, b) => -(a.order - b.order))
    const prevCard = prevCards.length > 0 ? prevCards[0] : null
    this._prevCardView = this._cardViews.find((i) => i.model == prevCard)

    const targetX =
      this._stackView.x + this._stackView.cardOffsetX * this._model.order
    const targetY =
      this._stackView.y + this._stackView.cardOffsetY * this._model.order
  ...
  update() {
    ...
    if (this._following) {
      this.x = this._prevCardView.x + this._stackView.cardOffsetX
      this.y = this._prevCardView.y + this._stackView.cardOffsetY
